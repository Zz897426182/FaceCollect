...
public class FtpRegister implements Serializable {
    ...
    public void registFtp() {
        if (!pathExists(CollectProperties.getRegisterPath())) {
            try {
                zooKeeper.create(rootPath, null, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
        ...
        try {
            if (!pathExists(rootPath + "/" + proxyPath)) {
                zooKeeper.create(rootPath + "/" + proxyPath, null, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
